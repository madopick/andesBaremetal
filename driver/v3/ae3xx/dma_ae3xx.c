...
#define DMA_CHANNEL(n)  ((DMA_CHANNEL_REG *)&(AE3XX_DMA->CHANNEL[n]))
...
int32_t dma_channel_configure (	uint8_t            ch,
	...
	AE3XX_DMA->INTSTATUS = (1U << (16+ch));
	AE3XX_DMA->INTSTATUS = (1U << (8+ch));
	AE3XX_DMA->INTSTATUS = (1U << ch);
...
void dma_irq_handler (void) {
	...
	for (ch = 0; ch < DMA_NUMBER_OF_CHANNELS; ch++) {
		if ((AE3XX_DMA->INTSTATUS & (1U << ch)) == 0) {
			dma_ch = DMA_CHANNEL(ch);

			// Terminal count request interrupt
			if (AE3XX_DMA->INTSTATUS & (1U << (16 + ch))) {
				...
				// DMA abort interrupt
				} else if (AE3XX_DMA->INTSTATUS & (1U << (8 + ch))) {
					dma_ch->CTRL = 0U;
					// Clear Channel active flag
					clear_channel_active_flag (ch);
