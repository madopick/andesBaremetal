...
static void gpio_set_intr_mode(uint32_t intr_mode, uint32_t mask) {
	...
	for (i = 0; i < GPIO_MAX_PIN_NUM; i++) {
		if (mask & (0x1 << i)) {
			if ((i >= 0) & (i < 8)) {
				AE3XX_GPIO->INTRMODE0 &= ~(7 << (i * 4));
				AE3XX_GPIO->INTRMODE0 |= (intr_mode << (i * 4));
			} else if ((i >= 8) & (i < 16)) {
				AE3XX_GPIO->INTRMODE1 &= ~(7 << ((i-8) * 4));
				AE3XX_GPIO->INTRMODE1 |= (intr_mode << ((i-8) * 4));
			} else if ((i >= 16) & (i < 24)) {
				AE3XX_GPIO->INTRMODE2 &= ~(7 << ((i-16) * 4));
				AE3XX_GPIO->INTRMODE2 |= (intr_mode << ((i-16) * 4));
			} else if ((i >= 24) & (i < 32)) {
				AE3XX_GPIO->INTRMODE3 &= ~(7 << ((i-24) * 4));
				AE3XX_GPIO->INTRMODE3 |= (intr_mode << ((i-24) * 4));
			}
		}
	}
}
...
void gpio_irq_handler() {
	...
	status = AE3XX_GPIO->INTRSTATUS;

	// write 1 to clear interrupt status
	AE3XX_GPIO->INTRSTATUS = status;
...
int32_t gpio_initialize(NDS_GPIO_SignalEvent_t cb_event) {
	...
	AE3XX_GPIO->INTREN = 0;
        ...
        // Write 1 to clear interrupt status
        AE3XX_GPIO->INTRSTATUS = 0xffffffff;
...
void gpio_pinwrite(uint32_t pin_num, int32_t val) {
	val ? (AE3XX_GPIO->DOUTSET = 1UL << pin_num) : (AE3XX_GPIO->DOUTCLEAR = (1UL << pin_num));
}

uint8_t gpio_pinread(uint32_t pin_num) {
	return ((AE3XX_GPIO->DATAIN & (1UL << pin_num)) ? (1) : (0));
}

void gpio_write(uint32_t mask, uint32_t val) {
	val ? (AE3XX_GPIO->DOUTSET = mask) : (AE3XX_GPIO->DOUTCLEAR = mask);
}

uint32_t gpio_read(void) {
	return (AE3XX_GPIO->DATAIN);
}

void gpio_setdir(uint32_t mask, int32_t dir) {
	dir ? (AE3XX_GPIO->CHANNELDIR |= mask) : (AE3XX_GPIO->CHANNELDIR &= ~mask);
}
