...
int32_t rtc_initialize(NDS_RTC_SignalEvent_t cb_event)
{
	...
	Tmp_C = AE3XX_RTC->CTRL;
	Tmp_S = AE3XX_RTC->STATUS;

	// disdable all interrupt and clear interrupt status
	AE3XX_RTC->CTRL = (Tmp_C & (~ BITS(0,7)));
	AE3XX_RTC->STATUS = (Tmp_S | BITS(2,7));
...
int32_t rtc_control(uint32_t control, uint32_t arg)
{
	...
	case NDS_RTC_CTRL_EN:
		// RTC ON
		if (arg == 1) {
			AE3XX_RTC->CTRL = (Tmp_C | CTRL_RTC_EN);
		// RTC OFF
		} else if (arg == 0) {
			AE3XX_RTC->CTRL = (Tmp_C & ~CTRL_RTC_EN);
		}
	...
	case NDS_RTC_CTRL_ALARM_WAKEUP:
		// INT ON
		if (arg == 1) {
			AE3XX_RTC->CTRL = (Tmp_C | CTRL_ALARM_WAKEUP);
		// INT OFF
		} else if (arg == 0) {
			AE3XX_RTC->CTRL = (Tmp_C & ~CTRL_ALARM_WAKEUP);
		}
	...
	case NDS_RTC_CTRL_ALARM_INT:
		// write 1 clear
		AE3XX_RTC->STATUS = (Tmp_S | STATUS_ALARM_INT);

		// INT ON
		if (arg == 1) {
			AE3XX_RTC->CTRL = (Tmp_C | CTRL_ALARM_INT);
		// INT OFF
		} else if (arg == 0) {
			AE3XX_RTC->CTRL = (Tmp_C & ~CTRL_ALARM_INT);
		}
	...
	case NDS_RTC_CTRL_DAY_INT:
		// write 1 clear
		AE3XX_RTC->STATUS = (Tmp_S | STATUS_DAY);

		// INT ON
		if (arg == 1) {
			AE3XX_RTC->CTRL = (Tmp_C | CTRL_DAY);
		// INT OFF
		} else if (arg == 0) {
			AE3XX_RTC->CTRL = (Tmp_C & ~CTRL_DAY);
		}
	...
	case NDS_RTC_CTRL_HOUR_INT:
		// write 1 clear
		AE3XX_RTC->STATUS = (Tmp_S | STATUS_HOUR);

		// INT ON
		if (arg == 1) {
			AE3XX_RTC->CTRL = (Tmp_C | CTRL_HOUR);
		// INT OFF
		} else if (arg == 0) {
			AE3XX_RTC->CTRL = (Tmp_C & ~CTRL_HOUR);
		}
	...
	case NDS_RTC_CTRL_MIN_INT:
		// write 1 clear
		AE3XX_RTC->STATUS = (Tmp_S | STATUS_MIN);

		// INT ON
		if (arg == 1) {
			AE3XX_RTC->CTRL = (Tmp_C | CTRL_MIN);
		// INT OFF
		} else if (arg == 0) {
			AE3XX_RTC->CTRL = (Tmp_C & ~CTRL_MIN);
		}
	...
	case NDS_RTC_CTRL_SEC_INT:
		// write 1 clear
		AE3XX_RTC->STATUS = (Tmp_S | STATUS_SEC);

		// INT ON
		if (arg == 1) {
			AE3XX_RTC->CTRL = (Tmp_C | CTRL_SEC);
		// INT OFF
		} else if (arg == 0) {
			AE3XX_RTC->CTRL = (Tmp_C & ~CTRL_SEC);
		}
	...
	case NDS_RTC_CTRL_HSEC_INT:
		// write 1 clear
		AE3XX_RTC->STATUS = (Tmp_S | STATUS_HSEC);

		// INT ON
		if (arg == 1) {
			AE3XX_RTC->CTRL = (Tmp_C | CTRL_HSEC);
		// INT OFF
		} else if (arg == 0) {
			AE3XX_RTC->CTRL = (Tmp_C & ~CTRL_HSEC);
		}
...
NDS_RTC_STATUS rtc_get_status(void)
{
	...
	rRtc_Status.Status = AE3XX_RTC->STATUS;
...
void rtc_period_irq_handler()
{
	...
	Tmp_W = Tmp_S = (AE3XX_RTC->STATUS & BITS(3,7));

	// write 1 clear all interrupt status
	AE3XX_RTC->STATUS = Tmp_W;
...
void rtc_alarm_irq_handler()
{
	...
	Tmp_W = Tmp_S = (AE3XX_RTC->STATUS & BIT(2));
